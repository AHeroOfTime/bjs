...
const diff = formatDistance(new Date(), new Date(2021, 3, 4, 10, 32, 0), {
  addSuffix: true,
});
// console.log(diff);
...
const person1 = { name: 'aaron' };
const person2 = { name: 'aaron' };
...
async function checkName() {
  const [err, successValue] = await to(checkIfNameIsCool('aaron'));
  if (err) {
    // deal with it
    console.log(err);
  } else {
